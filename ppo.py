    ...
    def steps(self, agent_policy, agent_value):
        ...
        for s in range(self.n_steps):
            ...
            if done:
                ...
                memories.append(
                    Memory(obs=self.obs, action=action, new_obs=new_obs, reward=reward, done=done, value=state_value,
                           adv=0))
...
def log_policy_prob(mean, std, actions):
    ...
    act_log_softmax = -((mean - actions) ** 2) / (2 * torch.exp(std).clamp(min=1e-4)) - torch.log(
...
if __name__ == '__main__':
    ...
    writer_name = 'PPO_' + ENV_NAME + '_' + date_time + '_' + str(POLICY_LR) + '_' + str(VALUE_LR) + '_' + str(
        TRAJECTORY_SIZE) + '_' + str(BATCH_SIZE)
    writer = SummaryWriter(log_dir='content/runs/' + writer_name)

    # create the test environment
    test_env = gym.make(ENV_NAME)
    if save_video_test:
        test_env = gym.wrappers.Monitor(test_env, "VIDEOS/TEST_VIDEOS_" + writer_name,
    ...
    while n_iter < MAX_ITER:
        ...
        for s in range(PPO_EPOCHS):
            ...
            for mb in range(0, len(batch), BATCH_SIZE):
                ...
                pol_loss, val_loss = clipped_PPO_loss(mini_batch, agent_policy, agent_value, minib_old_log_policy,
                                                      minib_adv, CLIP_EPS, writer, device)
